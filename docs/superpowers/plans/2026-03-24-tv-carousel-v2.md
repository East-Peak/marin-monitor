...
<div bind:this={containerEl} class="h-full overflow-hidden relative">
	{#if needsScroll}
		<div class="auto-scroll-track" style="animation-duration: {duration}s;">
...
<script lang="ts">
	...
	const weatherCache = new Map<
		string,
		{ data: { temp: number; wind: string; shortForecast: string }; fetchedAt: number }
	>();
			...
			<div class="grid grid-cols-2 gap-4">
				{#each items as item (item.id)}
					<div class="bg-gray-800/60 rounded-lg p-4 border border-gray-700/50">
						...
						<h3 class="text-lg font-semibold text-gray-100 leading-snug line-clamp-3">
							{item.title}
						</h3>
						{#if item.description}
