<script lang="ts">
	...
	const gasPct = $derived(snapshot?.fuelBreakdown.find((f) => f.fuelType === 'gasoline')?.pct ?? 0);
				...
				<div class="px-3 pt-3 pb-2 border-b border-gray-700/40">
					...
					<div class="grid grid-cols-3 gap-2 mt-2">
						<div class="text-center">
							<span class="text-lg font-bold tabular-nums" style="color: {INDIGO}"
								>{fmtCount(snapshot.totalVehicles)}</span
							>
					...
					<div class="w-full h-5 rounded overflow-hidden flex">
						{#each filteredFuel as fuel}
							...
							>
								{#if fuel.pct >= 6}
									<span
										class="flex items-center justify-center h-full text-[9px] font-bold text-white drop-shadow-sm"
									>
										{fuel.pct.toFixed(0)}%
									</span>
								{/if}
					...
					<div class="mt-1.5 grid grid-cols-2 gap-x-3 gap-y-0.5">
						{#each orderedFuel as fuel}
							<div class="flex items-center gap-1">
								...
								<span class="text-[9px] text-zinc-500 tabular-nums ml-auto shrink-0"
									>{fmtCount(fuel.count)}</span
								>
							</div>
						{/each}
					</div>
					...
					<div class="grid grid-cols-2 gap-1.5">
						<div class="rounded bg-zinc-700/30 px-2 py-1.5 text-center">
							<span class="text-lg font-bold tabular-nums text-white"
								>{fmtCount(snapshot.funStats.tesla)}</span
							>
							<p class="text-[9px] text-zinc-400">Teslas (#3 overall)</p>
						</div>
						<div class="rounded bg-zinc-700/30 px-2 py-1.5 text-center">
							<span class="text-lg font-bold tabular-nums text-white"
								>{fmtCount(snapshot.funStats.porsche)}</span
							>
							<p class="text-[9px] text-zinc-400">Porsches</p>
						</div>
						<div class="rounded bg-zinc-700/30 px-2 py-1.5 text-center">
							<span class="text-lg font-bold tabular-nums text-white"
								>{fmtCount(snapshot.funStats.rivian)}</span
							>
							<p class="text-[9px] text-zinc-400">Rivians</p>
						</div>
						<div class="rounded bg-zinc-700/30 px-2 py-1.5 text-center">
							<span class="text-lg font-bold tabular-nums" style="color: {INDIGO}"
								>{fmtCount(snapshot.funStats.lucid)}</span
							>
							<p class="text-[9px] text-zinc-400">Lucids</p>
						</div>
					</div>
					<div
						class="mt-1.5 rounded px-2 py-1.5 text-center"
						style="background: rgba(6, 182, 212, 0.1);"
					>
						<span class="text-base font-bold tabular-nums" style="color: #06b6d4"
							>{fmtCount(snapshot.funStats.hydrogen)}</span
						>
						<span class="text-[9px] text-zinc-400 ml-1"
							>Hydrogen FCEVs (yes, {numberToWord(snapshot.funStats.hydrogen)})</span
						>
					</div>
				</div>
			</div>
			...
			>
				...
				<div class="flex-1 px-3 py-2 overflow-hidden">
					{#each snapshot.topMakes.slice(0, 10) as make, i}
						...
						<div class="flex items-center gap-2 py-[3px]">
							<span class="text-[10px] text-zinc-500 w-4 text-right shrink-0 tabular-nums"
								>#{i + 1}</span
							>
							<span class="text-xs font-medium text-zinc-200 w-24 shrink-0 truncate"
								>{make.make}</span
							>
				...
				<div class="px-3 py-2 border-t border-gray-700/40">
					<p class="text-[8px] text-zinc-600 text-center">
						Source: California DMV Vehicle Fuel Type Count by Zip Code ({snapshot.dataYear}) via
						data.ca.gov
					</p>
				</div>
			</div>
