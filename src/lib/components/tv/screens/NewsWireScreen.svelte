      ...
      <div class="grid grid-cols-2 gap-3">
        {#each items as item, i (item.id + '-' + i)}
          <div class="rounded-lg border border-gray-700/50 bg-gray-800/60 p-3.5">
            ...
            <h3 class="line-clamp-2 text-[1.05rem] font-semibold leading-snug text-gray-100">{item.title}</h3>
            {#if item.description}
              <p class="mt-1 text-[0.92rem] leading-snug text-gray-400 line-clamp-2">{item.description}</p>
            {/if}
          </div>
        {/each}
      </div>
