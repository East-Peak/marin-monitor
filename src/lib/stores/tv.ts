...
import { safetyNews, localNews, civicNews, alerts, threeOneOneNews } from '$lib/stores/news';
...
export function buildIdxTickerItems(sources: IndexDataSources): TickerItem[] {
	...
	if (sources.grocery?.current) {
		...
		if (snap.totalCheapest != null) {
			const expStr =
				snap.totalExpensive != null ? ` – up to ${fmtLarge(snap.totalExpensive)} at priciest` : '';
			candidates.push({
				id: 'IDX-grocery',
				badge: 'IDX' as TickerCategory,
				category: 'IDX',
