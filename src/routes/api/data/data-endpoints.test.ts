...
// failure, non-OK response) they return 503 with a structured error body —
// see src/lib/server/blob-endpoint.ts.
//
...
describe('/api/data/311', () => {
	...
	it('returns 503 blob_unavailable when blob is missing', async () => {
		mockBlobMissing();
		const response = await get311({} as any);
		await expectStructured503(response, 'blob_unavailable');
	});

	it('returns 503 blob_fetch_failed when blob fetch returns non-OK', async () => {
		mockBlobFetchFail();
		const response = await get311({} as any);
		await expectStructured503(response, 'blob_fetch_failed');
	});
...
describe('/api/data/composite', () => {
	...
	it('returns 503 blob_unavailable when blob missing', async () => {
		mockBlobMissing();
		const response = await getComposite({} as any);
		await expectStructured503(response, 'blob_unavailable');
	});

	it('returns 503 blob_fetch_failed when fetch returns non-OK', async () => {
		mockBlobFetchFail();
		const response = await getComposite({} as any);
		await expectStructured503(response, 'blob_fetch_failed');
	});
...
describe('/api/data/cappuccino', () => {
	const samplePayload = {
		...
		history: [{ medianPrice: 5.2, timestamp: '2026-03-31T00:00:00Z' }]
	};
...
describe('/api/data/grocery-basket', () => {
	const samplePayload = {
		...
		history: [{ totalCost: 86.3, timestamp: '2026-03-31T00:00:00Z' }]
	};
